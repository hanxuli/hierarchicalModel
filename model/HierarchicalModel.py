...
from keras.layers import LSTM, Embedding, Input, Convolution1D, GlobalMaxPooling1D, Dropout, Concatenate, TimeDistributed, \
    Dense, \
...
filter_lengths = [3,4,5] # CNN c卷积核大小
nb_filter = 100  # 卷积神经网络积极核函数的个数
fc_hidden_dims = 128 # 全连接层隐藏单元数量
batch_size = 32
...
class data_loader(object):
    ...
    def load_datasets(self):
        ...
        f = open('../data/'+self.dataname+'/trainset.save', 'rb')
        trainset = cp.load(f,encoding='iso-8859-1')
        f = open('../data/'+self.dataname+'/devset.save', 'rb')
        devset = cp.load(f,encoding='iso-8859-1')
        f = open('../data/'+self.dataname+'/testset.save', 'rb')
    ...
    def load_embedding_matrix(self):
        ...
        f = open('../data/'+self.dataname+'/embinit.save', 'rb')
...
def get_model(
    ...
    embedding_sequences=Dropout(0.5)(embedding_sequences)
    ...
    fc_out = Dense(units=fc_hidden_dims, activation='tanh',
